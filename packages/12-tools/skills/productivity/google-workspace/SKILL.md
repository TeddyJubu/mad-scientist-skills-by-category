...
## Headless Server / CLI Auth

When doing Google OAuth on a headless VPS (no browser), the `--auth-url` then
`--auth-code` flow is the ONLY reliable approach. Key pitfalls discovered in
practice:

- **Do NOT use `rclone` for Google Drive on headless servers.** rclone's OAuth
  server hardcodes port 53682 and fails to bind if anything else uses it
  (common — the gateway backend often claims 53682). Even with workarounds,
  `rclone authorize` is unreliable: cloudflared tunnels return 502 errors,
  and the auth flow silently drops tokens.
- **Refresh token missing?** Google will NOT issue a refresh token if the user
  previously authorized the app without `access_type=offline` (the default).
  The fix: user must go to https://myaccount.google.com/permissions, find the
  app, click **Revoke access**, then re-authorize with `--auth-url`. Adding
  `&access_type=offline&prompt=consent` alone won't help if a session cookie
  remembers the old consent.
- **cloudflared quick tunnels for OAuth are unreliable.** They frequently
  return 502 Bad Gateway because timing issues between the tunnel connecting
  and the OAuth server listening. The `--auth-url` / `--auth-code` pattern
  bypasses this entirely — the server captures the code from the URL bar,
  exchanges tokens server-side, no tunnel needed for the callback.
- **Always tell users to copy the code from the address bar.** After clicking
  Allow, the user gets a "connection refused" page (expected — there's nothing
  running on localhost). The authorization code is in the address bar
  (`?code=4/0A...`). This always works, regardless of port conflicts or
  tunnel failures.
...
| `rclone authorize` fails on port bind | Abandon rclone. Use this skill's `--auth-url` / `--auth-code` flow instead |
| Google returns no refresh token | User must revoke existing auth at myaccount.google.com/permissions, then re-authorize |
