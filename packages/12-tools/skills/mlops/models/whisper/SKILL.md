...
## Video Transcription from Google Drive (Full Pipeline)

Use this when Charles (or anyone) shares a Google Drive video link and wants timestamped bullet points.

### Step 1: Install dependencies

```bash
pip install faster-whisper gdown --break-system-packages -q
...
```bash
# Extract file ID from the share link, then:
gdown "https://drive.google.com/uc?id=FILE_ID_HERE" -O /tmp/video_to_transcribe.mp4
```

### Step 3: Extract audio with ffmpeg

```bash
ffmpeg -i /tmp/video_to_transcribe.mp4 -vn -acodec pcm_s16le -ar 16000 -ac 1 /tmp/audio_transcribe.wav -y
```
...
### Step 5: Group into timestamped bullet points
...
CHUNK_SIZE = 60  # seconds per bullet
chunks, current_chunk, chunk_start = [], [], None
...
for seg in segments:
    ...
    if seg["end"] - chunk_start >= CHUNK_SIZE:
        ...
        current_chunk, chunk_start = [], None
...
- `base` model works well for English Zoom calls; use `small` or `medium` for heavy accents or noisy audio
- `vad_filter=True` is critical — skips silence and speeds up transcription significantly
- For a 1.5-hour video, expect ~8-10 minutes on CPU with `base`
- pip install requires `--break-system-packages` on this server (Ubuntu system Python)
- Output the `.txt` file as a Telegram document attachment
