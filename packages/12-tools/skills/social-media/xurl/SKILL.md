...
### When to use xitter vs xurl

**Use `xurl`** for any new X/Twitter work — it covers the full v2 API, OAuth 2.0 PKCE, media upload, DMs, and multi-account workflows.

**Use `xitter`** (older third-party x-cli wrapper) only if you need the specific `x-cli` shortcuts and the user has already set up `x-cli` credentials. xitter has a different credential format (5 individual env vars vs xurl's OAuth 2.0) and covers a smaller API surface. xitter SKILL.md has been archived as `references/xitter-legacy.md`.
