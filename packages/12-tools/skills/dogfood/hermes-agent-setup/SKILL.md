...
### Pairing codes not found or expired

Symptom: `hermes pairing approve telegram CODE` always returns "not found or expired" even with fresh codes.

Key facts about the pairing system:
- Pending codes stored in: `~/.hermes/pairing/telegram-pending.json`
- Approved users stored in: `~/.hermes/pairing/telegram-approved.json`
- Rate limits stored in: `~/.hermes/pairing/_rate_limits.json`
- Code TTL is 1 HOUR (not short) — expiry is rarely the real cause
- Rate limit is 10 min per user, max 3 pending codes per platform
- Each team profile (Bob, Mark, Michael, Tammie) has its OWN pairing store at `~/.hermes/profiles/<name>/platforms/pairing/`

Diagnostic steps:
1. `cat ~/.hermes/pairing/telegram-pending.json` — if empty `{}`, codes are never being written, meaning the gateway isn't receiving the request
2. Check rate limits: `cat ~/.hermes/pairing/_rate_limits.json` — a `_failures:telegram` count or a rate-limited user_id means requests are being silently dropped
3. `hermes gateway status` — confirm the main gateway is running
4. Check gateway logs: `journalctl --user -u hermes-gateway --no-pager -n 100 | grep -i pair`

Most likely cause when pending file stays empty: the new user is messaging the WRONG bot. With multiple team profiles running (Bob, Mark, Michael, Tammie each have their own Telegram bot), the user must message the specific bot tied to the main gateway, not a team profile bot. Ask which Telegram bot username they're messaging.
